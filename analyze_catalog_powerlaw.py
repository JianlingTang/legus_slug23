...
import emcee
from catalog_reader import reader_register
...
class libwgts(object):
    def __init__(self, p, mid = True , logmB = 10.): # for pure powerlaw, set mBreak = 10^10 Msun, making exponential goes to 1
        self.alphaM = p[0]
        self.mBreak = 10.**logmB
        self.mid = mid
        if self.mid:
            self.alphaT = p[1]
            self.tMid   = 10.**p[2]
...
def lnprob(params):
    ...
    if params[0] < -4.0 or \
       params[0] > 0.0 :
        # alpha_M in [-4, 0], log m_Break is 10 
        logL = -np.inf
    if not args.mdd:
...
parser.add_argument("comp_path",
                    help="path of cluster_slug library completenesses to use")
...
parser.add_argument("-nparam", "--nparam", type=int, default=3,
                    help="number of parameters to fit in powerlaw model"
                    "despite the paramters of the A_V distribution")
...
# if using completeness calculator ;
# for cat in catalogs:
#     cat['libcomp'] = []
#     for comp, d in zip(cat['comp'], cat['filtersets_detect']):
#         phot_tmp = phot_neb_ex[:,d]
#         cat['libcomp'].append(comp.comp(phot_tmp))
#         keep = np.logical_or(keep, cat['libcomp'][-1] > 0.0)
# if using pre-computed library completeness ;
for cat in catalogs:
    cat['libcomp'] = []
    for d in cat['filtersets_detect']:
        if '628c' in cat['basename'] : 
            if np.sum(d) == 5:
                ...
                keep = np.logical_or(keep,cat['libcomp'][-1] > 0.0)
                print("Loaded library for NGC 628c with all 5 bands.")
            elif np.sum(d) == 4:
                cat['libcomp'].append(np.load('libngc628cnoUV_4dim_comp_Mar_11.npy'))
                keep = np.logical_or(keep,cat['libcomp'][-1] > 0.0)
                print("Loaded library for NGC 628c with 4 bands without UV.")
            else :
            ...
            if  np.sum(d) == 5:
                ...
                keep = np.logical_or(keep,cat['libcomp'][-1] > 0.0)
                print("Loaded library for NGC 628e with all 5 bands.")
            elif  np.sum(d) == 4:
                cat['libcomp'].append(np.load('libngc628enoUV_4dim_comp_14_Mar.npy'))
                keep = np.logical_or(keep,cat['libcomp'][-1] > 0.0)
                print("Loaded library for NGC 628e with 4 bands without UV.")
            else :
                print('Completeness not valid')
                exit(0)
...
ndim = 3+args.nav   # Number of free parameters in priors
...
if args.restart:
    ...
    # if args.pl : 
    #     p0[:,1] = 10.  # log Mbreak = 10 for pure powerlaw
    # else :
    #     p0[:,1] = 6.0 + 2.0*(rand(args.nwalkers)-0.5)  # log Mbreak = 5 - 7
    if not args.mdd:
        p0[:,1] = -1.0 + 2.0*(rand(args.nwalkers)-0.5) # alphaT = -2 to 0
        p0[:,2] = 7.0 + 2.0*(rand(args.nwalkers)-0.5)  # log Tmid = 6 - 8
    else:
        p0[:,1] = 0.5 + 0.5*(rand(args.nwalkers)-0.5)  # gammaMdd = 0.25 - 0.75
        p0[:,2] = 5.0 + 2.0*(rand(args.nwalkers)-0.5)  # log Tmddmin = 4 - 6
